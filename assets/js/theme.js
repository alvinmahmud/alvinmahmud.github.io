...
let setThemeSetting = (themeSetting) => {
  ...
  applyTheme();
  updateThemeToggleLabel(themeSetting);
};

let updateThemeToggleLabel = (themeSetting) => {
  const themeToggle = document.getElementById("light-toggle");
  if (!themeToggle) return;

  const nextTheme = themeSetting == "dark" ? "light" : "dark";
...
// Determine the expected state of the theme toggle. Default to dark on the
// first visit, then preserve the user's explicit selection.
...
// The selected setting is always the computed dark or light theme.
...
let initTheme = () => {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    updateThemeToggleLabel(determineThemeSetting());
